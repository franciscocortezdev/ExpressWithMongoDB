...
export const getAllTasks = async (idUser: string) => {
  const responseItem = await taskModel.find({idUser})
  return responseItem
}
...
export const updateOneTask = async (id: string, data: TaskInterface, idUser:string) => {
  const responseItem = await taskModel.findOneAndUpdate({ _id: id , idUser}, data, {
    new: true,
  });
  return responseItem;
};
